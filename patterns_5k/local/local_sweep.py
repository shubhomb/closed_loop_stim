#!/usr/bin/env python3
"""
Local multi-GPU hyperparameter sweep orchestrator.
...
Usage
-----
    # Two GPUs, all configs in the YAML, one fold per config (from yaml)
    ...
    python local/local_sweep.py --sweep-config slurm/slurm_sweep_base_cnn.yaml \
...
def _expand_trials(sweep_cfg: dict, n_seeds: int, start_seed: int,
                   k_cross: Optional[int]) -> list[dict]:
    """Return a list of trial dicts (overrides + seed/fold + name)."""
    ...
    for idx, overrides in enumerate(override_list):
        ...
        if k_cross is not None:
            for fold in range(k_cross):
                name = _make_experiment_name(idx, overrides) + f"_fold{fold}"
                trials.append({
                    "overrides": overrides, "seed": base_seed,
                    "cv_fold": fold, "k_cross": k_cross,
...
def main():
    ...
    parser.add_argument("--seeds", type=int, default=5,
                        help="Seeds per config (ignored if k_cross is set).")
    parser.add_argument("--start-seed", type=int, default=0)
    parser.add_argument("--k-cross", type=int, default=None,
                        help="CV folds; overrides --seeds and yaml k_cross.")
    ...
    trials = _expand_trials(sweep_cfg, args.seeds, args.start_seed, k_cross)
    n_repeats = k_cross if k_cross else args.seeds
    n_configs = len(trials) // max(n_repeats, 1)

    log.info(f"Sweep: {sweep_name}  (local, grid)")
    if k_cross:
        log.info(f"Configs: {n_configs}  x  {k_cross} folds  =  {len(trials)} trials")
    else:
    ...
    for trial in trials:
        ...
        cfg["seed"] = trial["seed"]
        if "cv_fold" in trial:
            cfg["cv_fold"] = trial["cv_fold"]
            cfg["k_cross"] = trial["k_cross"]
        cfg["skip_oracle_plots"] = True
