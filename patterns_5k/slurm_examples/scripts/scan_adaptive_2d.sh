...
TARGETS=(4 8 16 32)
# TAU_VALUES=(0.2 0.05 0.02 0.01) # T5 5finger
# TAU_VALUES=(0.2 0.05 0.03 0.02) # T16 5finger
TAU_VALUES=(0.2 0.05 0.03 0.02) # T16 202505
    ...
    for ((k=0; k<${#TARGETS[@]}; k++)); do
      ...
      for ((seed=START_SEED; seed<=END_SEED; seed++)); do
        ...
        CMD="python -m optlearn.main \
          ...
          encoder.use_dof='[${DOF1},${DOF2}]' \
          task.n_targets=${target} \
          task.test_trials=10 \
          sampling.sampler.tau=${tau} \
          random_seed=${seed}"

        echo "Submitting: DOF=[${DOF1},${DOF2}], Targets=${target}, Tau=${tau}, Seed=${seed}"
