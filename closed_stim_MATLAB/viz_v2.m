...
binEdges   = -0.5:1:40.5;         % integer spike counts 0..30
binCenters = 0:40;
maxHalfWidth    = 0.35;           % max half width of violin
smoothSigmaBins = 2.0;            % smoothing in "bins"
nKernelRadius   = 1;              % +/- bins for Gaussian kernel

xLowCenter  = 1.0;
xHighCenter = 2.6;     % gap between low and high
intraGap    = 0.8;    % spacing within each group
...
% --- Jitter & Point Settings ---
jitterWidth = 0.15;      % Control horizontal spread of points (0 = vertical line)
pointSize = 12;          % Size of individual data points
pointAlpha = 0.25;       % Transparency of points to avoid clutter
...
for ch = recordedChannels
    ...
    if ch_num <= length(tc_main) && ~isempty(tc_main{ch_num})
        ...
        for sp = 1:4
            ...
            % 1. PLOT JITTERED POINTS
            % Generate random horizontal displacement centered at xPos(sp)
            xJitter = xPos(sp) + (rand(size(v)) - 0.5) * jitterWidth;
            
            scatter(xJitter, v, pointSize, colorRGB, 'filled', ...
                'MarkerFaceAlpha', pointAlpha, 'MarkerEdgeAlpha', 0, ...
                'HandleVisibility', 'off'); % Points won't clutter the legend

            % 2. DRAW VIOLIN DENSITY (Using your conv logic)
            ...
            plot([xPos(sp)-0.14, xPos(sp)+0.14], [mu, mu], '-', ...
                'Color', [0 0 0 0.85], 'LineWidth', 2.5, 'HandleVisibility', 'off');       
        end
        ...
        yText = 39.0;  % adjust if needed (since ylim is [0 30])
...
if ~isempty(violations)
    ...
    disp('✓ Sanity check passed: all stimulations coincide with threshold crossings');
end
